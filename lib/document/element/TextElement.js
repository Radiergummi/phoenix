...
class TextElement extends Node {
  ...
  insertText ( offset, newText ) {
    this.textContent = this.textContent.slice( 0, offset ) + newText + this.textContent.slice( offset );
  }
  ...
  replaceText ( offset = 0, count = this.textContent.length, newText ) {
    ...
    this.textContent = this.textContent.replace( obsolete, newText );
  }
  ...
  substringText ( offset, count ) {
    ...
    this.textContent = this.textContent.replace( extraction, '' );
