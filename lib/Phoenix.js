...
const p = new Phoenix( {
  origins: './lib/reader/**/*.js'
} );

p
  .on( 'read:error', d => console.error( 'error while reading', d.error ) )
  .on( 'read:init', d => console.error( 'read init', d.reader.constructor.name ) )
...
p
  ...
  .on( 'parse:init', d => console.error( 'parse init', d.parser.constructor.name ) )
...
p.run()
  .then( d => console.log( `finish`, d[ 0 ][ 0 ] ) )
